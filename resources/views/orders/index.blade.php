...
<div class="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
    ...
    <div class="mt-5 flex flex-wrap gap-2"><a href="{{ route('orders.index') }}" class="filter-pill {{ !$status ? 'filter-pill-active':'' }}">Semua</a>@foreach(['diproses','siap_diambil','dikirim','selesai'] as $value)<a href="{{ route('orders.index',['status'=>$value]) }}" class="filter-pill {{ $status===$value ? 'filter-pill-active':'' }}">{{ ucwords(str_replace('_',' ',$value)) }}</a>@endforeach</div>
